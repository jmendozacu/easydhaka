...
if($flag){
	...
	class HTZ_Vendor_Block_Catalog_Product_List extends Mage_Catalog_Block_Product_Abstract
	{
		...
		protected function _getProductCollection()
		{
			...
			if(count($this->getVendorApprovedProductIds())>0){
				$vFilPro = $this->_productCollection->addAttributeToFilter('entity_id', array('nin'=> $this->getVendorApprovedProductIds())); 
				return $vFilPro;
			} else {
		...
		protected function getVendorApprovedProductIds()
		{
			if(!Mage::getStoreConfig('vendor/settings/product_active')){ return array(); }
			...
			if(count($collection)>0){
				foreach($collection as $_collection){
					if(!$_collection->getHtzVendorProduct()){
						$vProIds[] = $_collection->getId();
					}
				}
			}
			return $vProIds;
