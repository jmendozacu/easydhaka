...
if($flag){
	class HTZ_Vendor_Block_Adminhtml_Catalog_Category_Tabs
    extends Mage_Adminhtml_Block_Catalog_Category_Tabs
	{
	}
} else {
	 
	class HTZ_Vendor_Block_Adminhtml_Catalog_Category_Tabs extends Mage_Adminhtml_Block_Widget_Tabs
	{
		/**
		...
		protected function _prepareLayout()
		{
			...
			if(Mage::helper('vendor')->getUserType()=='2'){
				if($this->getRequest()->getParam('parent')){
					$flag = true;
				} elseif($categoryId = $this->getRequest()->getParam('id')) {
					$connection = Mage::getSingleton('core/resource')->getConnection('core_write');
					$tableName	= Mage::getConfig()->getTablePrefix().'htz_vendor_category';
					$vendorId	= Mage::getSingleton('admin/session')->getUser()->getUserId();
					$query		= "SELECT `category_id` FROM $tableName
										WHERE `vendor_id`=$vendorId
										AND `category_id`=$categoryId";
					$row		= $connection->query($query);
					$result		= $row->fetch(PDO::FETCH_ASSOC);
					if(is_array($result)){$flag = true;}
				}
			} else {
				$flag = true;
			}
			...
			if($flag){
				$this->addTab('vendor', array(
					'label'     => Mage::helper('catalog')->__('Vendor Category'),
					'content'   => $this->getLayout()->createBlock(
						'adminhtml/catalog_category_tab_vendor')->toHtml(),
				));
			}
