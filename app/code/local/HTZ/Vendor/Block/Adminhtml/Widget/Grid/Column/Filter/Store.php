...
if($flag){
	class HTZ_Vendor_Block_Adminhtml_Widget_Grid_Column_Filter_Store
		...
		extends Mage_Adminhtml_Block_Widget_Grid_Column_Filter_Abstract
	{
		...
		public function getHtml()
		{
			...
			foreach ($websiteCollection as $website) {
				if($flag && !in_array($website->getId(), $userWebsiteIds)){continue;}
				$websiteShow = false;
				foreach ($groupCollection as $group) {
					if($flag && !in_array($group->getId(), $userGroupIds)){continue;}
					if ($group->getWebsiteId() != $website->getId()) {
						continue;
					}
					$groupShow = false;
					foreach ($storeCollection as $store) {
						if($flag && !in_array($store->getId(), $userStoreIds)){continue;}
