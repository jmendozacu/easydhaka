...
if($flag){
	class HTZ_Vendor_Block_Adminhtml_Permissions_User_Edit_Tab_Roles
    extends Mage_Adminhtml_Block_Permissions_User_Edit_Tab_Roles
	{
	}
} else {

	class HTZ_Vendor_Block_Adminhtml_Permissions_User_Edit_Tab_Roles extends Mage_Adminhtml_Block_Widget_Grid
	{
		...
		protected function _prepareColumns()
		{
			...
			if($this->getRequest()->getParam('user_id')){
				$roleId = $this->_getSelectedRoles();
			} else {
				$roles = Mage::getModel('admin/role')->getCollection()->addFieldToFilter('user_type','vendor_user')->getData();
				if(count($roles)==1 && $roles[0]['role_id']){
					$roleId = array($roles[0]['role_id']);
				}
			}
