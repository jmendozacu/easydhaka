...
class Aitoc_Aitloyalty_Model_Observer
{
    ...
    public function onAdminhtmlBlockSalesruleActionsPrepareform($observer)
    {
        ...
        foreach ($fieldset->getSortedElements() as $element)
        {
            if ($element->getName() == 'simple_action')
            {
                ...
                $values[] = array('value' => 'by_fixed_surcharge', 'label' => Mage::helper('salesrule')->__('Fixed amount surcharge'));
                $values[] = array('value' => 'by_percent_surcharge', 'label' => Mage::helper('salesrule')->__('Percent of product price surcharge'));
                $values[] = array('value' => 'cart_fixed_surcharge', 'label' => Mage::helper('salesrule')->__('Fixed amount surcharge for whole cart'));
...
// Magento version < 1.4.2.0
        ...
        switch ($rule->getSimpleAction()) {
            ...
            case 'by_fixed_surcharge':
                ...
                //echo "$quoteAmount - $discountAmount - $baseDiscountAmount"; exit;
                break;
