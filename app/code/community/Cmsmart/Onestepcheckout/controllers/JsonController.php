...
class Cmsmart_Onestepcheckout_JsonController extends Mage_Core_Controller_Front_Action
{
    ...
    protected function _getShippingMethodsHtml()
    {
        ...
        $shippingMethods->setTemplate('cmsmart/onestepcheckout/onepage/shipping_method.phtml');
    ...
    public function commentAction()
    {
        ...
        if (!empty($comment)) {
            Mage::getSingleton('core/session')->setOnestepcheckoutOrderComment($comment);
        } else {
            Mage::getSingleton('core/session')->unsOnestepcheckoutOrderComment($comment);
        }
        return;
    }
	...
	public function removeProductAction(){
		$response = array(); 
		$response['error'] = 0; 
		$response['empty_cart'] = 0;
		$idProduct = $this->getRequest()->getParam('_idproduct');
		$cartHelper = Mage::helper('checkout/cart');
		$items = $cartHelper->getCart()->getItems();
		foreach ($items as $item) {
			if ($item->getItemId() == $idProduct) {
		...
		if($response['error']!=1){
			$response['message'] = Mage::helper('customer')->__('Your cart was removed successfully');
		}
		$totalItemsInCart = Mage::helper('checkout/cart')->getItemsCount();
		if($totalItemsInCart==0){
			$response['empty_cart'] = 1; 
			$response['message'] = Mage::helper('customer')->__('Your cart was removed successfully');
		}
	...
	public function getQtyStock($productId){
		...
		return $qtyStock = Mage::getModel('cataloginventory/stock_item')->loadByProduct($_product)->getQty();
	}
	public function updateProductAction(){
		$response = array();
		$response['error'] = 0;
		$response['empty_cart'] = 0;
		$productDatas = Mage::app()->getRequest()->getPost();
		$cart = Mage::getSingleton('checkout/cart'); //->getItemsCount(); 
		$items = $cart->getItems();
		try {
			foreach ($items as $item) {
				foreach($productDatas['cart'] as $productId => $productData){
					if($item->getId()==$productId){  // IS THIS THE ITEM WE ARE CHANGING? IF IT IS:
						if($item->getQty()!=$productData['qty']){
                            // var_dump($item->getSku());die;
                            $simpleProduct=Mage::getModel('catalog/product')->loadByAttribute('sku', $item->getSku());
								if($productData['qty'] <= $this->getQtyStock($simpleProduct->getId())){
									if($productData['qty']==0){
										$itemId = $item->getItemId();
										$cart->removeItem($itemId);
									}else{
										$item->setQty($productData['qty']);
									}
								}else{
									$response['error'] = 1;
									$response['message'] = Mage::helper('customer')->__('Product cannot be ordered in requested quantity.');
								}
		...
		if($response['error']!=1){
			$response['message'] = Mage::helper('customer')->__('Your cart was updated successfully');	
		}
